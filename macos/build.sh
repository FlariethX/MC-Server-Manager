#!/bin/bash
# Збірка MC Server Manager у вигляді .app та .dmg для macOS.
#
# Використання:
#   chmod +x build_macos.sh
#   ./build_macos.sh
#
# Вимоги: встановлений Python 3 (див. інструкцію в чаті) та Xcode Command Line Tools
# (для hdiutil/iconutil, які вже входять у macOS за замовчуванням).
...
if command -v create-dmg >/dev/null 2>&1; then
  # Красивіший варіант з іконкою та ярликом "перетягни в Applications"
  create-dmg \
    --volname "$APP_NAME" \
    --window-pos 200 120 \
    --window-size 640 400 \
...
if [ ! -f "$DMG_PATH" ]; then
  # Запасний варіант — стандартна утиліта macOS, завжди доступна
